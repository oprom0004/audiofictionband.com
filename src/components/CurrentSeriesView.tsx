...
import { CURRENT_SERIES_COPYWRITE, COMMON_COPYWRITE } from "../data/copywriting";
...
export default function CurrentSeriesView() {
  ...
  return (
    <div id="current-series-view" className="space-y-10">
      ...
      <section id="series-intro-block" className="space-y-4">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20">
          ...
          <span className="text-xs font-mono font-medium text-cyan-400 uppercase tracking-wider font-semibold">
            {CURRENT_SERIES_COPYWRITE.badge}
          </span>
        </div>
        <h1 className="text-3xl font-sans font-bold text-gray-100 tracking-tight">
          {CURRENT_SERIES_COPYWRITE.title}
        </h1>
        <p className="text-gray-400 text-sm leading-relaxed max-w-4xl" dangerouslySetInnerHTML={{ __html: CURRENT_SERIES_COPYWRITE.intro }} />
      </section>
      ...
      <section id="amperage-spec-explorer" className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          ...
          <div className="flex flex-col gap-2">
            {[1, 2, 3, 5, 6].map(amp => (
              ...
              >
                ...
                <span className="text-sm font-mono font-bold text-cyan-400">{amp}A</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="md:col-span-8 bg-[#121214] border border-gray-800 rounded-xl p-6 space-y-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 pb-4 border-b border-gray-800">
            <div>
              <span className="block text-[9px] font-mono text-cyan-400 uppercase tracking-widest">Target Application Scope</span>
            ...
            <div className="bg-[#161619] px-3 py-1.5 border border-gray-800 rounded font-mono text-right shrink-0">
              ...
              <span className="block text-sm text-cyan-400 font-bold font-mono">{currentSpecDetail.powerWatts}</span>
            </div>
          </div>
          ...
          <div className="bg-black/30 border border-gray-900 rounded-lg p-4 text-center space-y-2">
            ...
            <div className="h-32 flex items-center justify-center">
              <svg className="w-full h-full max-w-sm" viewBox="0 0 100 40">
                ...
                <text x="8" y="5" fill="#06b6d4" fontSize="3" textAnchor="end" fontWeight="bold">{activeAmperage}A</text>

                {/* Interactive Operating Point bubble */}
                <circle cx="90" cy="5" r="2" fill="#06b6d4" className="animate-ping" />
                <circle cx="90" cy="5" r="1.5" fill="#06b6d4" />
              </svg>
            </div>
      ...
      <section id="all-models-directory" className="space-y-6 pt-8 border-t border-gray-800">
        <div>
          <h3 className="text-lg font-sans font-bold text-gray-200 flex items-center gap-2">
            ...
            {COMMON_COPYWRITE.brandName} ETM 800V DC Power Supply Complete Specifications Matrix
          </h3>
                  ...
                  <tr key={m.id} className="hover:bg-gray-800/10 transition-colors">
                    ...
                    <td className="p-4 text-center">
                      <a
                        ...
                        target="_blank"
                        rel="noopener"
                        className="inline-flex items-center gap-1 text-[11px] font-semibold text-cyan-400 hover:text-cyan-300 underline decoration-cyan-500/20 hover:decoration-cyan-400/50 transition-colors whitespace-nowrap"
                      >
                        View {m.id} Spec ↗
                      </a>
                    </td>
      ...
      {/* Selection notes from whitepaper */}
      <section id="spec-technical-notes" className="bg-[#121214] border border-gray-800 rounded-xl p-5 space-y-4">
        <div className="flex items-center gap-2 border-b border-gray-800 pb-2">
          <span className="w-2 h-2 rounded-full bg-cyan-500"></span>
          <h4 className="text-xs font-mono uppercase text-gray-200 tracking-wider">Model Selection and Electrical Notes</h4>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          <div className="space-y-2 text-gray-400">
            <p className="leading-relaxed">
              {CURRENT_SERIES_COPYWRITE.techNotesCol1}
            </p>
          </div>
          <div className="space-y-2 text-gray-405">
            <p className="leading-relaxed">
              {CURRENT_SERIES_COPYWRITE.techNotesCol2}
            </p>
          </div>
        </div>
      </section>
